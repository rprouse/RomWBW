...
in accordance with the intentions of their creators.

If you are the owner of any component within the RomWBW project
distribution and you object to the use of your work, please the
project's maintainer for immediate resolution:
...
## Historical Preservation & Hardware Validation Notice

RomWBW is a low-level firmware, HBIOS, and operating system build 
environment designed strictly for modern retro-computing hardware 
development, diagnostic testing, and historical architecture
engineering.

1. Operating System Frameworks

   The core operating system environments generated by this build suite 
   (including Digital Research CP/M 2.2 and CP/M 3.0) utilize legacy 
   assets that have been officially released by their respective 
   intellectual property successors under modern, permissive open-source 
   frameworks (such as the 3-Clause BSD License). All custom HBIOS and 
   integration code written explicitly for the RomWBW ecosystem is 
   distributed freely under open-source terms.

2. Supplemental Testing & Validation Binary Targets

   Certain pre-compiled, historic 8-bit binaries (such as legacy text 
   editors, compilers, and interpreted BASIC environments) may be 
   included within compressed system disk images or release compilation 
   packages.

   * Engineering Intent: These legacy binaries are packaged strictly as 
     an educational and engineering convenience. They serve as objective 
     baseline workloads to validate hardware instruction sets, benchmark 
     Z80/Z180/Z280 CPU execution, profile serial terminal emulation 
     timings, and verify stable disk controller storage read/write 
     routines.

   * Non-Commercial Scope: This project is entirely non-commercial, 
     generates zero revenue, and does not charge for access to any 
     software assets. The included historical utilities represent
     obsolete 30-to-50-year-old architectures that have no modern
     commercial market or active retail presence.

3. Notice to Verified Intellectual Property Rights Holders

   The maintainers of this repository respect active copyright laws and 
   the modern evolution of legacy software licensing (such as the 
   open-sourcing of historic 8-bit architectures like GW-BASIC and 6502 
   Microsoft BASIC).

   If you are a verified copyright holder or authorized legal 
   representative of a legacy utility bundled within these testing images
   and object to its inclusion as a historical hardware validation 
   target, please contact the repository maintainer directly via GitHub 
   Issue or [$doc_authmail$](mailto:$doc_authmail$). Upon verification 
   of ownership, any requested assets will be promptly modified, 
   obfuscated, or completely extracted from future build configurations 
   without requiring formal platform or administrative escalation.
