...
;
	...
	RET
;
; PCF IS FINE, DS1307 ITSELF DIDN'T ACK
DS7_INIT_NOACK:			; EXIT
	CALL	PRTSTRD			; WITH
	PRTS(" NOT PRESENT$")		; ERROR
	PUSH	AF
	JR	RTC_INIT_ERR
;
RTC_INIT_FAIL:				; EXIT
	CALL	PRTSTRD			; WITH
	PRTS(" I2C FAILURE$")		; ERROR
	PUSH	AF			; MESSAGE
RTC_INIT_ERR:				; EXIT
	POP	AF			; WITH
	LD	A,ERR_NOHW		; ERROR
...
;	
	LD	A,DS7_WRITE	; SET SLAVE ADDRESS
        CALL	PCF_START_ADDR	; GENERATE START CONDITION
	CALL	PCF_WAIT_FOR_PIN; AND ISSUE THE SLAVE ADDRESS
	CALL	NZ,PCF_PINERR
;
	LD	A,00H		; REGISTER 00
        CALL	PCF_PUTBYTE_PIN	; PUT ADDRESS ON BUS
	CALL	NZ,PCF_PINERR
;
	LD	DE,5
...
;	CALL	PRTHEXBYTE
        ...
        CALL	PCF_PUTBYTE_ACK	; PUT DATA ON BUS
	CALL	NZ,PCF_ACKERR
	POP	BC
	DEC	HL
	DJNZ	DS7_SC1
...
;
        LD     	A,0
        CALL	PCF_PUTBYTE_PIN	; PUT ADDRESS MSB ON BUS
	CALL	NZ,PCF_PINERR
;
...
DS7_RL1:CALL	PCF_READI2C
	...
	DJNZ    DS7_RL1
;
	LD     	A,PCF_ES0	; PREP NEGATIVE ACK FOR THE 8TH (LAST) BYTE
	OUT    	(PCF_RS1),A
	CALL	PCF_WAIT_FOR_PIN; WAIT FOR IT -- NOT PCF_READI2C, ITS LRB
	CALL	NZ,PCF_PINERR	; CHECK WOULD MISFIRE ON OUR OWN NACK
	IN	A,(PCF_RS0)	; READ THE FINAL BYTE, NACK ALREADY SENT
	LD	(HL),A
;
#IF (0)
	LD	A,8
...
#ENDIF
;
; BOUNDED (WAS AN INFINITE LOOP -- COULD HANG FOREVER IF PIN NEVER CLEARS)
; RETURNS Z/A=00 IF PIN CLEARED, NZ/A=FF ON TIMEOUT.
DS7_WTPIN:
	PUSH	HL
	LD	HL,PCF_PINTO
DS7_WTPIN0:
	...
	JR	Z,DS7_WTPIN_OK	; CLEAR -- DONE
	DEC	HL
	LD	A,H
	OR	L
	JR	NZ,DS7_WTPIN0	; KEEP WAITING IF NOT TIMED OUT
	; TIMED OUT
	POP	HL
	CALL   	PCF_STOP
	; TIMEOUT, FORCE NZ
