...
#DEFINE SYSCHKERR(HB_ERR) \
  ...
  #DEFCONT \	OR	A
;;;;
;;;; HANDLE DRIVER CHECK ERRORS
;;;;
;;;#DEFINE DRVCHKERR(DRV_ERR) \
;;;  #DEFCONT \	CALL	DRVCHKA
;;;  #DEFCONT \	LD	A,DRV_ERR
;;;  #DEFCONT \	OR	A
;
...
;
	...
	CALL	NEWLINE
	PRTS("DIO>$")
	CALL	SNAP			; DUMP REGS, NONE DESTROYED
;
...
HB_DSKREAD_NEW:
	...
	POP	HL			; RECOVER HL
;
	CALL	HB_DSKREAD00		; DO THE WORK
	RET	Z                       ; RETURN IF OK
	LD	D,$00			; SIGNAL READ
	JP	DSK_SHOWERR		; EXPOSE ERROR & RETURN
;
HB_DSKREAD:
	; THE ACTUAL SECTOR READ FUNCTION ADDRESS IS ON TOS, SAVE IT
	EX	(SP),HL			; SAVE HL TO TOS, HL := READ FN ADR
...
HB_DSKWRITE_NEW:
	...
	POP	HL			; RECOVER HL
;
	CALL	HB_DSKWRITE00		; DO THE WORK
	RET	Z			; RETURN IF OK
	LD	D,$01			; SIGNAL WRITE
	JP	DSK_SHOWERR		; EXPOSE ERROR & RETURN
;
HB_DSKWRITE:
	; THE ACTUAL SECTOR READ FUNCTION ADDRESS IS ON TOS, SAVE IT
...
;
	...
	LD	B,E			; SEC LOOP COUNTER
	LD	C,0			; SEC COMPLETE COUNTER
	LD	(HB_IOBUF),HL		; SAVE TGT BUF ADR
;
	;;;CALL	PC_SPACE
	;;;LD	A,'X'
	;;;CALL	COUT
...
HB_DSKIO1:
	; CALL READ/WRITE FN
	LD	HL,(HB_IOBUF)		; WRITE FROM WORK BUFFER
	PUSH	BC			; SAVE COUNTERS
...
;
	; INC TGT BUF ADR
	LD	HL,HB_IOBUF+1		; POINT TO MSB OF BUFFER ADR
	INC	(HL)			; BUMP DMA BY
	INC	(HL)			; ... 512 BYTES
;
	; INC SECTOR COUNT
...
;
HB_DSKIOX:
	LD	HL,(HB_IOBUF)		; UPDATED BUF ADR
	LD	E,C			; WRITE COUNT TO E
	OR	A			; SET RESULT FLAGS
	RET				; DONE
;
;--------------------------------------------------------------------------------------------------
; SHOW ACTIVE DRIVER ERROR MESSAGE
;--------------------------------------------------------------------------------------------------
;
DSK_SHOWERR:
	PUSH	AF			; SAVE STATUS
	...
	CP	4			; X < 4 ?
	JR	C,DSK_SHOWERR5		; SKIP IF DIAG LVL < 4
	POP	AF
	PUSH	AF
	PUSH	DE
	PUSH	HL
	CALL	DRV_PRTTAG		; PRINT UNIT PREFIX
	CALL	ERR_PRT_SP		; PRINT ERROR TEXT
	BIT	0,D			; WRITE?
	JR	NZ,DSK_SHOWERR1
	PRTS(" [READ$")
	JR	DSK_SHOWERR2
DSK_SHOWERR1:
	PRTS(" [WRITE$")
DSK_SHOWERR2:
	LD	A,CFG_DSKADR		; OFFSET TO DISK ADR FIELD
	CALL	LDHLIYA			; HL := IY + A, REG A TRASHED
	CALL	LD32			; GET THE CAPACITY VALUE
	BIT	7,D			; LBA?
	JR	Z,DSK_SHOWERR3		; IF Z, HANDLE CHS
	RES	7,D			; CLEAR LBA BIT
	PRTS(" @ LBA=0x$")
	CALL	PRTHEX32		; PRINT HEX VALUE
	JR	DSK_SHOWERR4		; SKIP TO END
DSK_SHOWERR3:
	PRTS(" @ CHS=$")
	CALL	PRTHEXWORDHL
	CALL	PC_COLON
...
;
	PUSH	AF			; PRESERVE INCOMING AF VALUE
	LD	A,(CB_DIAGLVL)		; GET DIAGNOSTIC LEVEL
	CP	DL_INFO			; >= INFO LEVEL
	JR	C,SYSCHK3		; BYPASS SNAP IF NOT
	POP	AF			; RESTORE AF
	CALL	SNAP			; DUMP REGISTERS & STACK
	RET				; DONE
SYSCHK3:
	POP	AF			; RESTORE AF
	RET				; DONE
...
SYSHALT:
	...
	JR	$			; HALT
;
; PRINT DRIVER TAG & DEVICE NUMBER
; IY MUST POINT TO ACTIVE/STANDARD DRIVER CFG BLOCK
;
DRV_PRTTAG:	
	PUSH	AF
	PUSH	DE
	...
	LD	D,(IY+CFG_TAG+1)
	CALL	WRITESTR
	LD	A,(IY+CFG_DEVNUM)	; GET CURRENT DEVICE NUM
	CP	$FE			; NOT YET ASSIGNED?
	JR	Z,DRV_PRTTAG1		; SKIP DEV NUM IF SO
	CALL	PRTDECB
DRV_PRTTAG1:
	CALL	PC_COLON
	POP	DE
