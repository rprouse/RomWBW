...
DB_END:
	RET
;
; PRINT A STRING BY LOOKING UP THE STRING POINTER FROM A TABLE
; OF WORD POINTERS AT ADDRESS HL INDEX A
;
PRTLKUP:
	PUSH	AF
	PUSH	DE
...
;
; LOG A TEST POINT ON THE CONSOLE W/ PC VALUE
;
TSTPT:
	CALL	PRTSTRI
	.DW	STR_TSTPT
	EX	(SP),HL			; GET RET ADR / SAVE HL
	CALL	PRTHEXWORDHL
	EX	(SP),HL			; RESTORE RET ADR / HL
	RET
;
; LOG A TEST POINT ON THE CONSOLE W/ PC VALUE & REG/STK DUMP
;
TSTPTX:
	CALL	PRTSTRI
	.DW	STR_TSTPT
	...
	; FALL THRU
;
; DISPLAY REGISTER AND STACK SNAPSHOT TO CURRENT
; HBIOS CONSOLE
;
;    REG: AF=0068 BC=2200 DE=0000 HL=206A IX=05C5 IY=1E25
;    ALT: AF=0000 BC=0000 DE=0000 HL=0000
;    STK: =FDFA >10B4 >1BBB >07D9 >A857 >0037 >73ED >FE28 >E031
;
SNAPALTS	.EQU	FALSE
;
SNAP:
	; GYMNATSTICS TO SAVE SP ON STACK
	PUSH	HL			; SAVE HL, SP = SP - 2
	LD	HL,0			; ZERO HL
	ADD	HL,SP			; HL = SP - 2
	INC	HL			; ADJUST HL BACK
	INC	HL			; ... TO ORIGINAL SP VAL
	EX	(SP),HL			; STORE ORIG SP AND RECOVER HL

	; SAVE ALL REGISTERS TO STACK IN REVERSE OF
	; DESIRED PRINTING ORDER
#IF (SNAPALTS)
	EXX
	PUSH	HL
	PUSH	DE
...
;
	...
	PUSH	AF
;
	; INIT HL TO POINT TO TOP OF STACK
	LD	HL,0
	ADD	HL,SP
;
	; SETUP LOOP TO PRINT VALUES FROM STACK
	; THE FIRST 6 VALUES PRINTED WILL BE THE REGS
	; JUST PUSHED.  BEYOND THAT WILL BE THE CALL
	; STACK.
	LD	IX,SNAP_TAGS - 1
#IF	(SNAPALTS)
	LD	B,7 + 4 + 8		; 7 REGS + 4 ALT REGS + 8 STACK VALS
#ELSE
	LD	B,7 + 8			; 7 REGS + 8 STACK VALS
#ENDIF
SNAP1:
	CALL	PC_SPACE		; SAVES SPACE IN STING LITERALS
SNAP2:
	; DISPLAY TAG STRING FOR NEXT VALUE
	INC     IX			; BUMP POINTER (FLAGS UNTOUCHED)
	LD	A,(IX)			; FETCH BYTE
	BIT     7,A			; LAST CHAR?
	RES     7,A			; STRIP SENTINEL BIT (FLAGS NOT CHANGED)
	CALL    COUT			; PRINT IT (NO REGS CHANGED)
	JR      Z,SNAP2			; IF SENTINEL BIT NOT SET, LOOP
;	
	; DISPLAY NEXT WORD VALUE FROM STACK
	LD	E,(HL)			; FETCH LSB
	INC	HL			; BUMP
	LD	D,(HL)			; FETCH MSB
	INC	HL			; BUMP
	EX	DE,HL			; MOVE VAL TO HL
	CALL	PRTHEXWORDHL		; PRINT IT
	EX	DE,HL			; RESTORE DE/HL
	DJNZ	SNAP1			; LOOP BASED ON COUNT
;
	; RESTORE ALL REGISTERS
#IF (SNAPALTS)
	POP	AF			; DUMMY POP, ALTS NOT MODIFIED
	POP	AF			; DUMMY POP, ALTS NOT MODIFIED
	POP	AF			; DUMMY POP, ALTS NOT MODIFIED
	POP	AF			; DUMMY POP, ALTS NOT MODIFIED
#ENDIF
	...
	POP	IX
	POP	IY			; RESTORE IY			
	INC	SP			; BURN THE STACK ENTRY
	INC	SP			; CONTAINING ORIGINAL SP
	RET				; DONE
;
SNAP_TAGS:
	; COMPACT TAG STRINGS; HIGH BIT SIGNIFIES FINAL CHAR OF EACH TAG
	.DB	"\r\nREG: AF",'='+$80,"BC",'='+$80,"DE",'='+$80,"HL",'='+$80,"IX",'='+$80,"IY",'='+$80
#IF (SNAPALTS)
	.DB	"\r\nALT: AF",'='+$80,"BC",'='+$80,"DE",'='+$80,"HL",'='+$80
#ENDIF
	.DB	"\r\nSTK: SP",'='+$80,'>'+$80,'>'+$80,'>'+$80,'>'+$80,'>'+$80,'>'+$80,'>'+$80,'>'+$80
;
STR_TSTPT	.TEXT	"\r\n>>> Test Point @$"
;
